...
int main() {
    ...
    grayscale_boxblur_shared<<<grid1, block, block.x * block.y * sizeof(unsigned char), stream[1]>>>(g_d, s_d, cols, rows);
    ...
    cudaMemcpyAsync(&out+(rows * cols)/2, s_d, (rows * cols)/2, cudaMemcpyDeviceToHost, stream[1]);
