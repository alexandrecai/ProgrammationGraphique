...
int main() {
    ...
    grayscale_boxblur_shared<<<grid1, block, block.x * block.y * sizeof(unsigned char), stream[1]>>>(rgb_d+((rows*cols)*sizeof(int)/2), g_d, cols, rows/2);
    ...
    cudaMemcpyAsync(out+((rows * cols)/2*3), g_d, (rows * cols)/2, cudaMemcpyDeviceToHost, stream[1]);
