...
void init() {
    ...
    initialisation("lapin.obj",&NbSommets,&NbFacets);
    cout << "la" << endl;
    GLuint* indices = new GLuint[NbFacets*3];
    GLfloat* coordonnees = new GLfloat[NbSommets*3];
    float *bbox = new float[6];

    lecture("lapin.obj",NbSommets,NbFacets,coordonnees,indices,bbox);
    ...
    programID = LoadShaders("../Shaders/TransformVertexShader.vert", "../Shaders/SimpleFragmentShader.frag");


    glGenBuffers(3, vboid);
    glBindBuffer(GL_ARRAY_BUFFER, vboid[0]);
    glBufferData(GL_ARRAY_BUFFER, 3 * NbSommets * sizeof(float), coordonnees, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
    glEnableVertexAttribArray(0);


    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboid[2]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * NbFacets * sizeof(GLuint), indices, GL_STATIC_DRAW);
...
void Display(void) {
    ...
    glDrawElements(GL_TRIANGLES, 3 * NbFacets, GL_UNSIGNED_INT, NULL);
